...
# ============================
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '2017335429').split()] # Replace with the actual admin ID(s) to add
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1001893842833').split()]  # Channel id for auto indexing (make sure bot is admin)
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002386866175'))  # Log channel id (make sure bot is admin)
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1002386866175'')  # Bin channel id (make sure bot is admin)
MOVIE_UPDATE_CHANNEL = int(environ.get('MOVIE_UPDATE_CHANNEL', ''))  # Notification of those who verify will be sent to your channel
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1002386866175'))  # Premium logs channel id
auth_channel = environ.get('AUTH_CHANNEL', '-1002933031477')  # Channel/Group ID for force sub (make sure bot is admin)
DELETE_CHANNELS = [int(dch) if id_pattern.search(dch) else dch for dch in environ.get('DELETE_CHANNELS', '').split()]
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1002288870325')  # Support group id (make sure bot is admin)
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002386866175')  # Request channel id (make sure bot is admin)
eqst_channel = environ.get('REQST_CHANNEL_ID', '-1002386866175')  # Request channel id (make sure bot is admin), '-1001572271892').split()]
MULTI_FSUB = [int(channel_id) for channel_id in environ.get('MULTI_FSUB', '-1002933031477').split() if re.match(r'^-?\d+$', channel_id)]  # Channel for force sub (make sure bot is admin)
...
#Auto approve 
CHAT_ID = [int(app_chat_id) if id_pattern.search(app_chat_id) else app_chat_id for app_chat_id in environ.get('CHAT_ID', '-1002521169702').split()]
TEXT = environ.get("APPROVED_WELCOME_TEXT", "<b>{mention},\n\nʏᴏᴜʀ ʀᴇǫᴜᴇsᴛ ᴛᴏ ᴊᴏɪɴ {title} ɪs ᴀᴘᴘʀᴏᴠᴇᴅ.\n\‣ ᴘᴏᴡᴇʀᴇᴅ ʙʏ @Samirmeshram</b>")
...
COLLECTION_NAME = environ.get('COLLECTION_NAME', 'DpCinemaOfficial')
...
VERIFIED_LOG = int(environ.get('VERIFIED_LOG', '-1002386866175'))  # Log channel id (make sure bot is admin)
...
CHNL_LNK = environ.get('CHNL_LNK', 'https://t.me/DpCinemaOfficial')
OWNER_LNK = environ.get('OWNER_LNK', 'https://t.me/Samirmeshram')
MOVIE_UPDATE_CHANNEL_LNK = environ.get('MOVIE_UPDATE_CHANNEL_LNK', 'https://t.me/DpCinemaOfficial')
OWNERID = int(os.environ.get('OWNERID', '2017335429'))  # Replace with the actual admin ID
...
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '2017335429').split()]
...
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'https://t.me/+jvIRS9NBZ5M5MmZl')  # Support group link (make sure bot is admin)
...
PM_SEARCH = bool(environ.get('PM_SEARCH', True))  # PM Search On (false) / Off (False)
...
SESSION_NAME = str(environ.get('SESSION_NAME', 'Dpcinema'))
MULTI_CLIENT = False
name = str(environ.get('name', 'Samirmeshram'))
